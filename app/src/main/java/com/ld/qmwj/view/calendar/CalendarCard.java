...
public class CalendarCard extends View {
    ...
    private int mCellSpace_COL;     //单元横向间距
    private int mCellSpace_ROW;     // 单元纵向间距
    ...
    private Cell mClickCell;        //选中的cell
    private static CustomDate mClickDate;  //选中的日期
    ...
    private void fillDate() {
        ...
        for (int j = 0; j < TOTAL_ROW; j++) {
            ...
            for (int i = 0; i < TOTAL_COL; i++) {
                ...
                //判断是否是选中的日期
                if (mClickDate != null && rows[j].cells[i].date.equals(mClickDate) && mClickDate.getMonth() == mShowDate.getMonth()) {
                    rows[j].cells[i].state = State.TODAY;
                }
            }
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mCellSpace_COL = mViewWidth / TOTAL_COL;
        mCellSpace_ROW = mViewHeight / TOTAL_ROW;
        if (!callBackCellSpace) {
            callBackCellSpace = true;
        }
        mTextPaint.setTextSize(mCellSpace_ROW / 3);
    }
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (Math.abs(disX) < touchSlop && Math.abs(disY) < touchSlop) {
                    int col = (int) (mDownX / mCellSpace_COL);
                    int row = (int) (mDownY / mCellSpace_ROW);
    ...
    private void measureClickCell(int col, int row) {
        ...
        if (rows[row] != null) {
            ...
            //  判断点击的日期是否在当前日期之前
            CustomDate clickDate = new Cell(rows[row].cells[col].date,
                    rows[row].cells[col].state, rows[row].cells[col].i,
                    rows[row].cells[col].j).date;
            SimpleDateFormat sdf = new SimpleDateFormat("yyyy_MM_dd");
            boolean b = true;
            try {
                CustomDate nowCustom = new CustomDate();
                nowCustom.day += 1;
                Date nowDate = sdf.parse(nowCustom.toString());
                Date selectDate = sdf.parse(clickDate.toString());
                b = selectDate.before(nowDate);
                //Log.d(MainActivity.TAG,"比较："+mShowDate.toString()+" "+clickDate.toString()+" 结果"+b);
        ...
        public void drawSelf(Canvas canvas) {
            switch (state) {
                case TODAY: // 今天
                    ...
                    canvas.drawCircle((float) (mCellSpace_COL * (i + 0.5)),
                            (float) ((j + 0.5) * mCellSpace_ROW), mCellSpace_ROW / 3,
                            mCirclePaint);
                    break;
            ...
            canvas.drawText(content,
                    (float) ((i + 0.5) * mCellSpace_COL - mTextPaint
                            .measureText(content) / 2), (float) ((j + 0.7)
                            * mCellSpace_ROW - mTextPaint
